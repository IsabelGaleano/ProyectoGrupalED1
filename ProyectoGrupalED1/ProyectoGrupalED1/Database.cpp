...
void Database::insertarJugador(string correo, string nombre, string apellido, double puntaje) {
	...
	this->abrirConexion();
	if (this->conectar) {
		//string  insertar = "INSERT INTO JUGADORES(correo_electronico,nombre,apellido, puntaje) VALUES('" + jugador->getCorreoElectronico() + "','" + jugador->getNombre() + "','" + jugador->getApellido() + "','" + std::to_string(jugador->getPuntaje());
